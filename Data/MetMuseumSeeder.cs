    ...
    public class MetMuseumSeeder
    {
        public static async Task SeedAsync(ApplicationDbContext context)
        {
            ...
            foreach (var artistName in allowedArtists)
            {
                ...
                if (artistProfiles.ContainsKey(artistName))
                {
                    profileImg = artistProfiles[artistName].img;
                }
                // Instagram-style kullanıcı adı
                string profileUsername = "@" + new string(artistName.ToLower().Where(char.IsLetterOrDigit).ToArray());
            ...
            foreach (var keyword in keywords)
            {
                ...
                foreach (var id in objectIds)
                {
                    ...
                    string profileImg = "";
                    if (artistProfiles.ContainsKey(artist))
                    {
                        profileImg = artistProfiles[artist].img;
                        artistBio = artistProfiles[artist].bio;
                    }
                    decimal price = 10000 + (added * 500);
                    var category = context.Categories.FirstOrDefault() ?? new Category { Name = "Genel" };
